              ...
              <div class="flex justify-between items-start mb-3">
                <div class="flex-1">
                  ...
                  <p class="text-sm text-gray-600">
                    <span class="font-medium">Total:</span> \${{ formatCurrency(invoice.total_amount) }}
                  </p>
                  <p class="text-sm text-gray-600">
                ...
                <table class="w-full text-xs">
                  ...
                  <tbody class="divide-y divide-gray-200">
                    <tr *ngFor="let product of invoice.xml_products">
                      ...
                      <td class="px-2 py-1 text-right">{{ formatCurrency(product.precio) }}</td>
                      <td class="px-2 py-1 text-right font-semibold">{{ formatCurrency(product.total) }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
      ...
      reader.onload = (e: any) => {
        try {
          ...
          conceptos.forEach((concepto, index) => {
            ...
            // Extraer información fiscal del nodo cfdi:Traslado
            const impuestos = concepto.querySelector('Impuestos');
            const traslados = impuestos?.querySelector('Traslados');
            const traslado = traslados?.querySelector('Traslado');

            let subtotal = importeConcepto; // Por defecto, el importe es el subtotal
            let iva = 0;
            let totalConcepto = importeConcepto;

            if (traslado) {
              // Extraer Base (subtotal sin IVA) e Importe (IVA)
              subtotal = parseFloat(traslado.getAttribute('Base') || '0');
              iva = parseFloat(traslado.getAttribute('Importe') || '0');
              totalConcepto = subtotal + iva;
            }

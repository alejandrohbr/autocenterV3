...
export class PdfGeneratorService {
  ...
  generateDiagnosticBudgetHTML(order: Order, customer: Customer): string {
        ...
        if (isRecommended) {
          ...
          productosRecomendadosRows += `
            <tr>
              ...
              <td style="border: 1px solid #ddd; padding: 4px; text-align: right; font-size: 10px;">$${formatCurrencyValue(producto.precio)}</td>
              <td style="border: 1px solid #ddd; padding: 4px; text-align: right; font-size: 10px;">$${formatCurrencyValue(producto.precio * producto.cantidad)}</td>
            </tr>
          `;
          recomendadosIdx++;
        } else {
      ...
      order.servicios.forEach((servicio, idx) => {
        ...
        serviciosRows += `
          <tr>
            ...
            <td style="border: 1px solid #ddd; padding: 4px; text-align: right; font-size: 10px;">$${formatCurrencyValue(servicio.precio)}</td>
            <td style="border: 1px solid #ddd; padding: 4px; text-align: right; font-size: 10px;">$${formatCurrencyValue(servicio.precio)}</td>
          </tr>
        `;
      });
        ...
        if (auth.is_authorized) {
          ...
          authorizedAuthsRows += `
            <tr>
              ...
              <td style="border: 1px solid #ddd; padding: 4px; text-align: center; font-size: 10px;">1</td>
              <td style="border: 1px solid #ddd; padding: 4px; text-align: right; font-size: 10px;">$${formatCurrencyValue(auth.estimated_cost || 0)}</td>
              <td style="border: 1px solid #ddd; padding: 4px; text-align: right; font-size: 10px;">$${formatCurrencyValue(auth.estimated_cost || 0)}</td>
            </tr>
          `;
          authorizedIdx++;
        ...
        if (!authorizedItemIds.has(item.id || '') && !item.isRejected) {
          ...
          diagnosticoRows += `
            <tr>
              ...
              <td style="border: 1px solid #ddd; padding: 4px; text-align: right; font-size: 10px;">$${formatCurrencyValue(item.estimatedCost || 0)}</td>
            </tr>
          `;
          itemIdx++;
        }
    ...
    // Procesar facturas XML agrupadas por factura
    let facturasXmlSection = '';
    let subtotalFacturasXml = 0;
    let totalIvaFacturas = 0;
    let totalConIvaFacturas = 0;

    if (order.xmlProducts && order.xmlProducts.length > 0) {
      // Agrupar productos por número de factura
      const facturasPorNumero: { [key: string]: any[] } = {};

      order.xmlProducts.forEach(product => {
        const facturaNum = product.invoice_folio || 'Sin Factura';
        if (!facturasPorNumero[facturaNum]) {
          facturasPorNumero[facturaNum] = [];
        }
        facturasPorNumero[facturaNum].push(product);
      });

      // Generar HTML para cada factura
      Object.keys(facturasPorNumero).forEach((facturaNumero) => {
        const productos = facturasPorNumero[facturaNumero];
        ...
        productos.forEach((producto, idx) => {
          const prodSubtotal = producto.subtotal || 0;
          const prodIva = producto.iva || 0;
          const prodTotal = producto.total || 0;

          subtotalFactura += prodSubtotal;
          ivaFactura += prodIva;
          totalFactura += prodTotal;

          productosRows += `
            <tr>
              <td style="border: 1px solid #ddd; padding: 3px; text-align: center; font-size: 9px;">${idx + 1}</td>
              <td style="border: 1px solid #ddd; padding: 3px; font-size: 9px;">${producto.descripcion}</td>
              <td style="border: 1px solid #ddd; padding: 3px; text-align: center; font-size: 9px;">${producto.cantidad}</td>
              <td style="border: 1px solid #ddd; padding: 3px; text-align: right; font-size: 9px;">$${formatCurrencyValue(producto.precio)}</td>
              <td style="border: 1px solid #ddd; padding: 3px; text-align: right; font-size: 9px;">$${formatCurrencyValue(prodSubtotal)}</td>
              <td style="border: 1px solid #ddd; padding: 3px; text-align: right; font-size: 9px;">$${formatCurrencyValue(prodIva)}</td>
              <td style="border: 1px solid #ddd; padding: 3px; text-align: right; font-size: 9px; font-weight: bold;">$${formatCurrencyValue(prodTotal)}</td>
            </tr>
          `;
        });

        subtotalFacturasXml += subtotalFactura;
        totalIvaFacturas += ivaFactura;
        totalConIvaFacturas += totalFactura;

        facturasXmlSection += `
          <div style="margin-bottom: 15px; border: 1px solid #d1d5db; border-radius: 4px; overflow: hidden;">
            <div style="background: #3b82f6; color: white; padding: 6px 10px; display: flex; justify-content: space-between; align-items: center;">
              <div>
                <strong style="font-size: 11px;">Factura: ${facturaNumero}</strong><br>
                <span style="font-size: 9px;">Proveedor: ${primerProducto.proveedor}</span>
              </div>
              <div style="text-align: right; font-size: 9px;">
                <div>Subtotal: $${formatCurrencyValue(subtotalFactura)}</div>
                <div>IVA (16%): $${formatCurrencyValue(ivaFactura)}</div>
                <div style="font-weight: bold; font-size: 10px;">Total: $${formatCurrencyValue(totalFactura)}</div>
              </div>
            </div>
            <table style="width: 100%; border-collapse: collapse;">
              <thead>
                <tr>
                  <th style="background: #60a5fa; color: white; padding: 4px; text-align: center; border: 1px solid #3b82f6; font-size: 9px; width: 30px;">#</th>
                  <th style="background: #60a5fa; color: white; padding: 4px; text-align: left; border: 1px solid #3b82f6; font-size: 9px;">DESCRIPCIÓN</th>
                  <th style="background: #60a5fa; color: white; padding: 4px; text-align: center; border: 1px solid #3b82f6; font-size: 9px; width: 50px;">CANT.</th>
                  <th style="background: #60a5fa; color: white; padding: 4px; text-align: right; border: 1px solid #3b82f6; font-size: 9px; width: 70px;">PRECIO</th>
                  <th style="background: #60a5fa; color: white; padding: 4px; text-align: right; border: 1px solid #3b82f6; font-size: 9px; width: 70px;">SUBTOTAL</th>
                  <th style="background: #60a5fa; color: white; padding: 4px; text-align: right; border: 1px solid #3b82f6; font-size: 9px; width: 70px;">IVA (16%)</th>
                  <th style="background: #60a5fa; color: white; padding: 4px; text-align: right; border: 1px solid #3b82f6; font-size: 9px; width: 80px;">TOTAL</th>
                </tr>
              </thead>
              <tbody>
                ${productosRows}
    ...
    const total = subtotalProductos + subtotalServicios + subtotalAuthorizedAuths + totalConIvaFacturas;
    ...
    return `
      ...
      <body>
        ...
        ${facturasXmlSection ? `
          <div class="section-title">📄 Productos de Facturas XML</div>
          ${facturasXmlSection}
          <div style="background: #e0f2fe; border: 2px solid #0284c7; border-radius: 4px; padding: 8px; margin-top: 10px;">
            <div style="display: flex; justify-content: space-between; font-size: 11px; margin-bottom: 4px;">
              <span style="font-weight: bold;">Total Subtotal (Sin IVA):</span>
              <span>$${formatCurrencyValue(subtotalFacturasXml)}</span>
            </div>
            <div style="display: flex; justify-content: space-between; font-size: 11px; margin-bottom: 4px;">
              <span style="font-weight: bold;">Total IVA (16%):</span>
              <span>$${formatCurrencyValue(totalIvaFacturas)}</span>
            </div>
            <div style="display: flex; justify-content: space-between; font-size: 13px; font-weight: bold; border-top: 2px solid #0284c7; padding-top: 4px;">
              <span>TOTAL FACTURAS XML:</span>
              <span>$${formatCurrencyValue(totalConIvaFacturas)}</span>
            </div>
          </div>
        ` : ''}
        ...
        <div class="totals-box">
          ${subtotalProductos > 0 ? `
            <div class="total-row">
              ...
              <span>$${formatCurrencyValue(subtotalProductos)}</span>
            </div>
          ` : ''}
          ${(subtotalServicios > 0 || subtotalAuthorizedAuths > 0) ? `
            <div class="total-row">
              <span>Subtotal Mano de Obra:</span>
              <span>$${formatCurrencyValue(subtotalServicios + subtotalAuthorizedAuths)}</span>
            </div>
          ` : ''}
          ${totalConIvaFacturas > 0 ? `
            <div class="total-row">
              <span>Subtotal Facturas XML:</span>
              <span>$${formatCurrencyValue(totalConIvaFacturas)}</span>
            </div>
          ` : ''}
          ${!isAuthorized && (subtotalProductosRecomendados > 0 || subtotalDiagnostico > 0 || subtotalPendingAuths > 0) ? `
            <div class="total-row" style="color: #666; font-style: italic; font-size: 10px;">
              ...
              <span>$${formatCurrencyValue(subtotalProductosRecomendados + subtotalDiagnostico + subtotalPendingAuths)}</span>
            </div>
          ` : ''}
